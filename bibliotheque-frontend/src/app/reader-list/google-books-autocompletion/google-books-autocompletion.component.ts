...
import { GoogleBooksService } from 'src/app/backend/google-books.service';
...
export class GoogleBooksSearchComponent {
  ...
  @Output() autocompleteItemSelected = new EventEmitter<Book>();

  constructor(private googleBooksService: GoogleBooksService) { }
  
  public async searchBooks(input: string): Promise<void> {
    this.searchResults = (await this.googleBooksService.booksAutocompletion(input));
  }

  public displayBookTitle(book: Book): string {
    return book?.title;
  }
  ...
  public getCover(book: Book): string {
    return book?.picture || this.DEFAULT_COVER_URL;
  }
}
